...
import platform
import k4a_camera
from k4a_camera import k4a_camera as k4a
K4A = 1
RS2 = 2
class VideoRecorder():  
    "Video class based on openCV"
    def __init__(self, name="temp_video.avi", compress = False, camindex=0, fps=30):

        self.camera = k4a( k4a_camera.RES_2160P, k4a_camera.FPS_30)
        colour, depth = self.camera.read()
        print(type(colour))
        colour_width = colour.shape[1]
        colour_height = colour.shape[0]

        # depth_width = depth.shape[0]
        # depth_height = colour.shape[1]

        self.fourcc = "MJPG"
        self.open = True
        self.device_index = camindex
        # self.fps = self.camera.get_fps()                  # fps should be the minimum constant rate at which the camera can
        self.fps = 30
